...
| Spark-X2.5 | Spark-X2.5-4B / 1.7B (XHToken) | Accepted (headwise sigmoid attention output gate `self_attn.g_proj`, 3:1 sliding-512/full layers with per-layer RoPE theta + partial rotary on the full layers, GeGLU, tied embeddings) — **not yet box-tested**; run `qlora_validate_native.py` first, with at least one prompt longer than 512 tokens so the sliding window is exercised |
