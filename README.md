...
- **Real-time (inference-time) training**: one loaded model serves generation and trains its adapter in place (`exllamav3.training.RealtimeQLoRA`) — `ingest()` a batch of samples — SFT rows and TRL explicit-prompt DPO pairs / KTO rows in one call, same losses as the offline preference trainer, frozen base as the reference — and the updated adapter is live in the very next generation (in-memory sync into the runtime LoRA slots; KV cache invalidated on update, or target only non-KV projections and skip invalidation). Constant externally-settable LR, timestamped rollback checkpoints, a readers/writer lock separating serving from training. Interactive demo (`/learn`, `/prefer`, `/good`, `/bad`) + server-integration reference: `training/realtime_chat.py`.
